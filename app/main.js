...
const getBiometricPlugin = () => {
  // Prefer the official Aparajita Capacitor plugin name. Keep the legacy
  // custom bridge as a fallback for older native builds.
  return getNativePlugin("BiometricAuth") || getNativePlugin("BiometricAuthNative");
};
...
const googleDriveBridge = () => {
  try {
    return window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.GoogleDriveBackup
      ? window.webkit.messageHandlers.GoogleDriveBackup
      : null;
  } catch (_) { return null; }
};
const sendGoogleDriveAction = (action, json = null) => {
  const bridge = googleDriveBridge();
  if (!bridge) return false;
  const payload = { action };
  if (json != null) payload.json = json;
  try { bridge.postMessage(payload); return true; } catch (_) { return false; }
};
const [googleDriveConnected, setGoogleDriveConnected] = useState(false);
React.useEffect(() => {
  const onGoogleDriveResult = e => {
    const detail = e && e.detail ? e.detail : {};
    if (detail.action === "status" || detail.action === "connect" || detail.action === "disconnect") {
      setGoogleDriveConnected(detail.connected === true);
    }
    if (detail.action === "connect" && detail.ok) {
      alert("Google Drive connected. You can now enable Automatic Google Drive backup.");
    } else if (detail.action === "connect" && !detail.ok) {
      alert(detail.error || "AleemFin could not connect to Google Drive.");
    } else if (detail.action === "disconnect" && detail.ok) {
  ...
  sendGoogleDriveAction("status");
  return () => window.removeEventListener("aleemfinGoogleDriveResult", onGoogleDriveResult);
}, []);
const connectGoogleDrive = () => sendGoogleDriveAction("connect");
const disconnectGoogleDrive = () => sendGoogleDriveAction("disconnect");
const uploadCurrentBackupToGoogleDrive = () => {
...
const getAutomaticBackup = () => {
  try {
    const saved = localStorage.getItem(AUTO_BACKUP_KEY);
    return saved ? JSON.parse(saved) : null;
  } catch (_) { return null; }
};
const exportAutomaticBackup = () => {
  const data = getAutomaticBackup();
  if (!data) { alert("No automatic backup has been created yet."); return; }
  const blob = new Blob([JSON.stringify(data, null, 2)], { type: "application/json" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = `aleemfin_auto_backup_${String(data.createdAt || todayISO()).slice(0,10)}.json`;
  a.click();
...
// One-time migration/reconciliation for loan-linked Ledger records. Older
// versions stored the account currency in the Ledger `amount`, which caused
// the Loans card and Home/Ledger view to drift after currency edits. The
// authoritative loan movement is now mirrored into the linked transaction,
// while `accountAmount` remains the exact balance-sheet impact in the account
// currency.
useEffect(() => {
  let changed = false;
  const reconciledLoans = loans.map(loan => {
    const movements = Array.isArray(loan.movements) ? loan.movements : null;
    if (!movements || movements.length === 0) return loan;
    const principalTotal = movements.filter(m => m.kind === "principal").reduce((sum, m) => sum + Number(m.amount || 0), 0);
    const repaidTotal = movements.filter(m => m.kind === "repayment").reduce((sum, m) => sum + Number(m.amount || 0), 0);
    const firstPrincipal = movements.find(m => m.kind === "principal");
    const next = {
      ...loan,
      amount: principalTotal,
      repaid: Math.min(repaidTotal, principalTotal),
      accountId: loan.accountId || firstPrincipal?.accountId || null,
      date: loan.date || firstPrincipal?.date || loan.date
    };
    if (JSON.stringify(next) !== JSON.stringify(loan)) changed = true;
    return next;
  });
  const reconciledTxns = transactions.map(tx => {
    if (!tx || !tx.loanId) return tx;
    const loan = reconciledLoans.find(l => String(l.id) === String(tx.loanId));
    if (!loan) return tx;
    let movement = Array.isArray(loan.movements) ? loan.movements.find(m => String(m.id) === String(tx.movementId)) : null;
    if (!movement) {
      const inferredKind = tx.category === "Loan Repayment" ? "repayment" : "principal";
      const txRaw = Number(tx.accountAmount != null ? tx.accountAmount : tx.amount) || 0;
      const txAccount = accounts.find(a => String(a.id) === String(tx.accountId));
      const txAccountCurrency = txAccount?.currency || tx.currency || loan.currency || "AED";
      const txLoanAmount = convertFromAED(convertToAED(txRaw, txAccountCurrency), loan.currency || txAccountCurrency);
      movement = Array.isArray(loan.movements) ? loan.movements.find(m =>
        m.kind === inferredKind &&
        String(m.accountId || "") === String(tx.accountId || "") &&
        m.date === tx.date &&
        Math.abs(Number(m.amount || 0) - txLoanAmount) < 1e-6
      ) : null;
    }
    if (!movement) return tx;
    const movementAmount = Number(movement.amount || 0) || 0;
    const account = accounts.find(a => String(a.id) === String(movement.accountId || tx.accountId));
    const accountId = movement.accountId || tx.accountId || null;
    const accountCurrency = account ? account.currency : (tx.accountAmount != null ? tx.currency : loan.currency);
    const accountAmount = account
      ? convertFromAED(convertToAED(movementAmount, loan.currency || "AED"), account.currency)
      : Number(tx.accountAmount != null ? tx.accountAmount : movementAmount) || 0;
    const next = {
      ...tx,
      amount: movementAmount,
      currency: loan.currency || tx.currency || "AED",
      rateToAED: exchangeRates[loan.currency || tx.currency || "AED"] || 1,
      accountId,
      accountAmount,
      movementId: movement.id || tx.movementId
    };
    if (JSON.stringify(next) !== JSON.stringify(tx)) changed = true;
    return next;
  });
  // Every movement has one Ledger view. Older cash-only movements may have
  // existed without a transaction, so create their missing view without
  // touching account balances because this is a reconciliation of history.
  const additions = [];
  reconciledLoans.forEach(loan => {
    (loan.movements || []).forEach(m => {
      const exists = reconciledTxns.some(t => String(t.loanId) === String(loan.id) && String(t.movementId || t.id) === String(m.id));
      if (exists) return;
      const account = m.accountId ? accounts.find(a => String(a.id) === String(m.accountId)) : null;
      const amount = Number(m.amount || 0);
      const accountAmount = account ? convertFromAED(convertToAED(amount, loan.currency || "AED"), account.currency) : null;
      additions.push({
        id: makeId(),
        title: m.kind === "repayment" ? `${loan.type === "lent" ? "Repayment from" : "Repayment to"} ${loan.name}` : `${loan.type === "lent" ? "Loan to" : "Loan from"} ${loan.name}`,
        type: m.kind === "repayment" ? (loan.type === "lent" ? "income" : "expense") : (loan.type === "lent" ? "expense" : "income"),
        category: m.kind === "repayment" ? "Loan Repayment" : "Loan", amount, currency: loan.currency || "AED", rateToAED: exchangeRates[loan.currency || "AED"] || 1,
        accountAmount, accountId: account?.id || null, date: m.date || loan.date || todayISO(), recordedAt: new Date().toISOString(), loanId: loan.id, movementId: m.id
      });
      changed = true;
    });
  });
  const finalTxns = additions.length ? [...additions, ...reconciledTxns] : reconciledTxns;
  if (changed) {
    setLoans(reconciledLoans);
    setTransactions(finalTxns);
    persistAllData(accounts, assets, reconciledLoans, finalTxns);
  }
  // Run once against the current stored state.
...
// Loan ledger helpers keep the loan object, its movement history, linked
// transactions, account balances, and all downstream calculations in sync.
const loanAmountFromTransaction = (tx, loan) => {
  if (!tx) return 0;
  // Loan-linked transactions store their debt movement in the loan currency
  // (`amount` + `currency`) and the actual account impact separately in
  // `accountAmount`. This keeps Ledger/Home and Loans as two views of one
  // relationship even when the account uses another currency.
  if (tx.movementId || tx.loanId) return Number(tx.amount || 0) || 0;
  const raw = Number(tx.accountAmount != null ? tx.accountAmount : tx.amount) || 0;
  const txCurrency = tx.accountId ? (accounts.find(a => a.id === tx.accountId)?.currency || tx.currency || loan.currency) : (tx.currency || loan.currency);
  return convertFromAED(convertToAED(raw, txCurrency), loan.currency || txCurrency);
};
const reverseLoanTransactionBalance = (accs, tx) => {
  if (!tx || !tx.accountId) return accs;
  const raw = Number(tx.accountAmount != null ? tx.accountAmount : tx.amount) || 0;
  return accs.map(acc => {
    if (acc.id !== tx.accountId) return acc;
    const delta = tx.type === "income" ? -raw : raw;
    return { ...acc, balance: acc.balance + delta };
  });
};
const applyLoanTransactionBalance = (accs, tx, amountInAccountCurrency) => {
  if (!tx || !tx.accountId) return accs;
  const raw = Number(amountInAccountCurrency) || 0;
  return accs.map(acc => {
    if (acc.id !== tx.accountId) return acc;
    const delta = tx.type === "income" ? raw : -raw;
    return { ...acc, balance: acc.balance + delta };
...
const [editingLoanMovementId, setEditingLoanMovementId] = useState(null);
const [repaymentModalLoan, setRepaymentModalLoan] = useState(null);
// Briefly holds the id of a loan that just transitioned to fully paid, so the
// Loans tab can play a one-time checkmark/settle animation on its card. Purely
// presentational — cleared automatically and never persisted.
const [justSettledLoanId, setJustSettledLoanId] = useState(null);
...
// Briefly holds the id of a goal that just crossed 100% funded, so the
// Planning tab can play a one-time celebration burst. Purely presentational.
const [celebrateGoalId, setCelebrateGoalId] = useState(null);
...
const openEditModal = (type, item) => {
  ...
  setModalType(type);
  // Build the base form before any Loan-specific branch uses it.
  // Home's Loan activity rows are represented as income/expense transactions;
  // entering the Loan branch must therefore never hit a temporal-dead-zone error.
  const base = getDefaultFormInput();
  if ((type === "income" || type === "expense") && item && item.loanId) {
    const loan = loans.find(l => String(l.id) === String(item.loanId));
    if (loan) {
      setEditingId(loan.id);
      setEditingLoanMovementId(item.movementId || item.id);
      const movement = (loan.movements || []).find(m => String(m.id) === String(item.movementId || item.id));
      const amount = movement ? movement.amount : item.amount;
      setModalType("loan");
      setFormInput({
        ...base,
        title: loan.name,
        amount: String(amount),
        currency: loan.currency || item.currency || "AED",
        loanType: loan.type,
        accountId: (movement && movement.accountId) || item.accountId || "",
        whatsapp: loan.whatsapp || "",
        dueDate: loan.dueDate || "",
        date: (movement && movement.date) || item.date || todayISO()
      });
      setModalOpen(true);
      return;
    }
  ...
  if (type === "account") {
    ...
    const primaryPrincipal = Array.isArray(item.movements)
      ? item.movements.find(m => m.kind === "principal")
      : null;
    const linkedPrincipalTx = transactions.find(t => String(t.loanId) === String(item.id) && (primaryPrincipal ? String(t.movementId) === String(primaryPrincipal.id) : t.category === "Loan"));
    const primaryAccountId = (primaryPrincipal && primaryPrincipal.accountId) || (linkedPrincipalTx && linkedPrincipalTx.accountId) || item.accountId || "";
    setFormInput({
      ...base,
      title: item.name,
...
const statementSortKey = tx => {
  if (tx?.recordedAt) {
    const parsed = Date.parse(tx.recordedAt);
...
const handleRepaymentSubmit = e => {
  ...
  try {
    saveStateToHistory();
    const wasFullyPaid = loanMetrics(loan).isFullyPaid;
    ...
    // Detect a loan crossing into "fully paid" from this repayment so the
    // Loans tab can play a one-time checkmark/settle animation on its card.
    const updatedLoan = result.loans.find(l => String(l.id) === String(loan.id));
    if (updatedLoan && !wasFullyPaid && loanMetrics(updatedLoan).isFullyPaid) {
      setJustSettledLoanId(updatedLoan.id);
      try { hapticFeedback(20); actionSound("success"); } catch (_) {}
      setTimeout(() => setJustSettledLoanId(id => id === updatedLoan.id ? null : id), 2000);
    }
...
const exportCSV = () => {
  const header = ["Date", "Title", "Type", "Category", "Amount", "Currency", "Account", "To Account", "To Amount", "To Currency", "Comment"];
  const rows = transactions.map(t => {
    const acc = accounts.find(a2 => a2.id === t.accountId);
    const toAcc = t.type === "transfer" ? accounts.find(a2 => a2.id === t.toAccountId) : null;
    return [t.date, t.title, t.type, t.category, t.amount, t.currency, acc ? acc.name : "", toAcc ? toAcc.name : "", t.type === "transfer" ? t.toAmount ?? t.amount : "", t.type === "transfer" ? t.toCurrency || t.currency : "", t.comment || ""];
  });
...
const saveGoal = e => {
  ...
  // Detect a goal crossing from "not funded" to "fully funded" on this save so
  // the Planning tab can play a one-time celebration burst on its card.
  const priorGoal = goalForm.id ? goals.find(item => item.id === goalForm.id) : null;
  const wasComplete = !!priorGoal && Number(priorGoal.targetAmount) > 0 && priorGoal.currentAmount >= priorGoal.targetAmount;
  const isComplete = goal.targetAmount > 0 && goal.currentAmount >= goal.targetAmount;
  if (!wasComplete && isComplete) {
    setCelebrateGoalId(goal.id);
    try { hapticFeedback(18); actionSound("success"); } catch (_) {}
    setTimeout(() => setCelebrateGoalId(id => id === goal.id ? null : id), 1800);
  }
  setPlanningEditor(null);
};
...
const removeCategory = (type, name) => {
  const linkedBudgets = type === "expense" ? budgets.filter(b => String(b.category || "").trim().toLowerCase() === String(name || "").trim().toLowerCase()) : [];
  if (linkedBudgets.length) {
    const label = linkedBudgets.length === 1 ? "budget" : "budgets";
    alert(`“${name}” is linked to ${linkedBudgets.length} ${label}. Remove or reassign ${label} before deleting this category so its spending history is not orphaned.`);
    return;
  }
