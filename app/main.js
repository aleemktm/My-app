...
const toggleTransactionFlag = tx => {
  const updated = transactions.map(item => String(item.id) === String(tx.id) ? { ...item, flagged: !item.flagged } : item);
  setTransactions(updated);
  persistAllData(accounts, assets, loans, updated);
  hapticFeedback(12);
};
...
const renderTxRow = tx => {
  ...
  return React.createElement(SwipeRow, {
    ...
    onLeftAction3: () => toggleTransactionFlag(tx),
    leftActionLabel: "Comment",
    leftAction2Label: tx.pinned ? "Unpin" : "Pin",
    leftAction3Label: tx.flagged ? "Unflag" : "Flag for review",
    leftActionKind: "comment",
    leftAction2Kind: "pin",
    leftAction3Kind: "flag",
