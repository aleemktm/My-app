...
(function () {
  function Planning(props) {
    ...
    const FormSheet = window.Modals && window.Modals.LoanFormSheet;

    const budgetSheet = planningEditor === "budget" && FormSheet ? h(FormSheet, {
      accent, accounts: props.accounts, darkMode, inputCls,
      title: budgetForm.id ? "Edit budget" : "New budget",
      submitLabel: budgetForm.id ? "Save changes" : "Save budget",
    ...
    const goalSheet = planningEditor === "goal" && FormSheet ? h(FormSheet, {
      accent, accounts: props.accounts, darkMode, inputCls,
      title: goalForm.id ? "Edit goal" : "New goal",
      submitLabel: goalForm.id ? "Save changes" : "Save goal",
      onClose: () => setPlanningEditor(null),
      onSubmit: saveGoal
    },
    ...
    return h("div", { className: "planning-premium max-w-2xl mx-auto w-full", style:{touchAction:"pan-y"} },
      ...
      h("div", { className:"loan-filter-segment planning-subtabs", role:"tablist", "aria-label":"Planning sections" },
        [["planning","Planning"],["recurring","Recurring"]].map(([value,label]) => h("button", {
          key:value, type:"button", role:"tab", "aria-selected":subTab===value,
          onClick:()=>setSubTab(value), className:`loan-filter-tab ${subTab===value?"is-active":""}`
        }, label))
      ),
      subTab === "planning" ? h(React.Fragment, null,
        ...
        h("section", { className: "planning-section" },
          ...
          budgets.length === 0 && h("div", { className: "planning-empty" }, h(Icons.IconTune, { className: "w-5 h-5" }), h("strong", null, "Give your spending a lane"), h("p", null, "Set a monthly limit for any expense category.")),
        ...
        h("section", { className:"planning-section" },
          ...
          goals.length===0 && h("div",{className:"planning-empty"},h(Icons.IconTarget,{className:"w-5 h-5"}),h("strong",null,"Turn a goal into a plan"),h("p",null,"Track the amount, target date and pace in one place.")),
