...
(function () {
  function PlanningContent(props) {
    ...
    return h("div", { className: "planning-premium max-w-2xl mx-auto w-full" },
      ...
      h("section", { className:"planning-section" },
        ...
        h("div",{className:"planning-list"},goals.map(goal=>{const progress=goal.targetAmount>0?Math.min(100,Math.round(goal.currentAmount/goal.targetAmount*100)):0;const monthly=goalMonthlyNeed(goal);const isCelebrating=celebrateGoalId!=null&&String(celebrateGoalId)===String(goal.id);return h(window.SwipeRow,{key:goal.id,selectionKey:selectionKey("goal",goal.id),onEdit:()=>openGoalEditor(goal),onDelete:()=>deleteGoal(goal)},h("article",{className:`planning-goal-card ${isCelebrating?"is-celebrating":""}`},h("div",{className:"planning-goal-head"},h("div",{className:`planning-goal-icon ${isCelebrating?"is-celebrating":""}`},h(Icons.IconTarget,{className:"w-5 h-5"}),isCelebrating&&h("span",{className:"goal-confetti","aria-hidden":"true"},[0,1,2,3,4,5,6,7].map(i=>h("i",{key:i,style:{"--i":i}})))),h("div",{className:"min-w-0 flex-1"},h("h4",null,goal.name),h("p",null,goal.targetDate?`Target ${dateFmt(goal.targetDate)}`:"No target date")),h("strong",null,`${progress}%`)),h("div",{className:"planning-progress goal-progress"},h("div",{style:{width:`${progress}%`}})),h("div",{className:"planning-item-meta"},h("span",null,`${goal.currency} ${numFmt(goal.currentAmount)} saved`),h("span",null,`${goal.currency} ${numFmt(goal.targetAmount)} target`)),h("div",{className:"planning-goal-footer"},monthly?h("span",null,`Need ~${goal.currency} ${numFmt(monthly.amount)}/mo`):h("span",null,goal.currentAmount>=goal.targetAmount?"Goal reached":"Add a target date"),h("span",{className:progress>=100?"goal-reached":""},progress>=100?"Complete":"In progress"))));}))
      )
    );
  }
  window.Tabs = window.Tabs || {};
  window.Tabs.PlanningContent = PlanningContent;
  ...
  function Planning(props) {
    ...
    const recurringView = typeof Recurring === "function"
      ? h(Recurring, { ...props, recurringItems: safeRecurring })
      : h("div", { className: "planning-empty" }, h("strong", null, "Recurring is unavailable"), h("p", null, "Please reload AleemFin."));

    return h("div", { className: "planning-premium max-w-2xl mx-auto w-full", style: { overflowX: "hidden", touchAction: "pan-y" } },
      h("header", { className: `planning-hero ${props.darkMode ? "is-dark" : ""}` },
        h("div", { className: "planning-hero-copy" },
          h("p", { className: "planning-eyebrow" }, "YOUR PLANNING"),
          h("h2", { className: "planning-title" }, "Planning & Recurring"),
          h("p", { className: "planning-subtitle" }, "Organise budgets, goals and predictable money in one place.")
        ),
        h("div", { className: "planning-hero-mark" }, h(Icons.IconTarget, { className: "w-6 h-6" }))
      ),
      ...
      subTab === "planning" ? h(PlanningContent, props) : recurringView
    );
  }
