...
, h(SettingsSection, {
 ...
 , h(SettingsSection, { title: "Security", subtitle: "Choose how AleemFin should protect your financial information on iPhone." }, h("div", { className: "space-y-2" },
    h(SettingsRow, { icon: Icons.IconSettings, title: "Face ID / Touch ID", detail: settings.biometricEnabled === true ? "Enabled · Native authentication will be used on supported iPhones." : "Use Face ID or Touch ID to unlock AleemFin." }, IOSSwitch({ checked: settings.biometricEnabled === true, onChange: () => updateSettings({ biometricEnabled: settings.biometricEnabled !== true }), label: "Face ID or Touch ID" })),
    ...
    h(SettingsRow, { icon: Icons.IconTune, title: "Allow notifications", detail: settings.notificationsEnabled === true ? "Notifications are enabled in AleemFin. iOS permission will be connected in the Capacitor phase." : "Turn on notifications to prepare AleemFin reminders." }, IOSSwitch({ checked: settings.notificationsEnabled === true, onChange: async () => { const next = settings.notificationsEnabled !== true; updateSettings({ notificationsEnabled: next }); if (next && window.AleemFinNative && window.AleemFinNative.requestNotifications) await window.AleemFinNative.requestNotifications(); }, label: "Allow notifications" })),
