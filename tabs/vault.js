...
(function () {
  ...
  function Vault(props) {
    const { applyLiveGoldRate, assets, darkMode, goldSyncMsg, liveGoldAEDPerGram, numFmt, openAddModal, openEditModal, setDeleteTarget, syncLiveGoldRate, syncingGold, selectionKey } = props;
    const total = assets.reduce((s,a)=>s+Number(a.currentPriceAED||0),0);
    const gain = assets.reduce((s,a)=>s+Number(a.currentPriceAED||0)-Number(a.purchasePriceAED||0),0);
    ...
    return h("div", { className: `assets-native ${darkMode ? "is-dark" : ""}` },
      ...
      h("section", { className:"assets-summary-grid" },
        h("div", { className:"assets-summary-card assets-summary-main" }, h("span",null,"Total value"), h("strong",null,"AED ",numFmt(total)), h("small",null,`${assets.length} tracked asset${assets.length===1?"":"s"}`)),
        h("div", { className:`assets-summary-card ${gain>=0?"is-positive":"is-negative"}` }, h("span",null,"Gain / loss"), h("strong",null,(gain>=0?"+":"-")+"AED "+numFmt(Math.abs(gain))), h("small",null,total?((gain/(total-gain||1))*100).toFixed(1)+"% change":"—")),
